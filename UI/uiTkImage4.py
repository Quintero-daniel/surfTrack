...
# Camera Settings
piCam = Picamera2()
piCam.preview_configuration.main.size=(480, 200)
...
#         current_image = Image.open('/home/dan/Downloads/surf.jpeg').resize((460, 200))
#         self.photo = ImageTk.PhotoImage(image=current_image)
        ...
        # state=tk.DISABLED
        self.scale_one = Scale(window, width=10, borderwidth=0, length=460, from_=0, to=255, tickinterval=0, orient=HORIZONTAL)
        self.scale_one.place(x=10, y=465)
        
        self.scale_two = Scale(window, width=10, borderwidth=0, length=460, from_=0, to=255, tickinterval=0, orient=HORIZONTAL)
        self.scale_two.place(x=10, y=500)
        
        self.scale_three = Scale(window, width=10, borderwidth=0, length=460, from_=0, to=255, tickinterval=0, orient=HORIZONTAL)
        self.scale_three.place(x=10, y=535)
        
        self.scale_four = Scale(window, width=10, borderwidth=0, length=460, from_=0, to=255, tickinterval=0, orient=HORIZONTAL)
        self.scale_four.place(x=10, y=570)
        
        self.scale_five = Scale(window, width=10, borderwidth=0, length=460, from_=0, to=255, tickinterval=0, orient=HORIZONTAL)
        self.scale_five.place(x=10, y=605)
        
        self.scale_six = Scale(window, width=10, borderwidth=0, length=460, from_=0, to=255, tickinterval=0, orient=HORIZONTAL)
        self.scale_six.place(x=10, y=640)
        
        self.rec_button_state = False
        self.rec_button = Button(text ="REC", fg='red', bg=self.default_color, command=self.rec_command)
        self.rec_button.place(x=420, y=690)       
        self.refresh_images()
    ...
    def check_buttons_pressed(self):
        if not self.color_image_checkbutton_var.get():
            ...
            self.color_image_label.place(x=0, y=10)
        else:
        ...
        if not self.mask_image_checkbutton_var.get():
            ...
            self.mask_image_label.place(x=0, y=240)
        else:
